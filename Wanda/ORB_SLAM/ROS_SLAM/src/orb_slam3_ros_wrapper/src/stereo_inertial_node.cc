...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber sub_img_left = node_handler.subscribe("/camera/left/image_raw", 100, &ImageGrabber::GrabStereoInertialLeft, &igb);
    ros::Subscriber sub_img_right = node_handler.subscribe("/camera/right/image_raw", 100, &ImageGrabber::GrabStereoInertialRight, &igb);

    setup_ros_publishers(node_handler);

    std::thread sync_thread(&ImageGrabber::SyncWithImuStereo, &igb);
