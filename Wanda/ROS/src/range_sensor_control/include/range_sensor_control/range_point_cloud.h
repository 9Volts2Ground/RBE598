#ifndef __RANGE_POINT_CLOUD_H__
#define __RANGE_POINT_CLOUD_H__

#include <string>
#include <vector>

// ROS libraries
#include <ros/ros.h>
#include <geometry_msgs/TransformStamped.h>
#include <sensor_msgs/PointCloud2.h>
#include <sensor_msgs/Range.h>
#include <tf2_ros/transform_listener.h>
#include <Eigen/Geometry>

// point cloud headers
#include <pcl/point_cloud.h>
#include <pcl_conversions/pcl_conversions.h>

// Custom libraries
#include "topic_messages/topics.h"

//=============================================================================
class range_point_cloud
{
public:
    // Constructor
    range_point_cloud( ros::NodeHandle* nh_in );

private:
    void process_arguments();

    void range_to_point_cloud( sensor_msgs::Range range );

    bool clutter_present( float range, float uncert_width);

    // Extracts a TF transformation
    void grab_transform(
        std::string frame_left,
        std::string frame_right,
        Eigen::Matrix3d& rot,
        Eigen::Vector3d& tran,
        ros::Time time_stamp );

    Eigen::Matrix3d rot_w2r; // Rotation matrix from world to range sensor frames
    Eigen::Vector3d tran_w2r; // Translation from world to range sensor frames

    tf2_ros::Buffer tfBuffer;
    tf2_ros::TransformListener tfListener;

    std::string sensor_frame;
    std::string world_frame;

    // Values used for filtering out ground clutter
    bool prevent_ground_clutter;
    std::string ground_frame;
    Eigen::Matrix3d rot_g2r; // Rotation matrix from world to range sensor frames
    Eigen::Vector3d tran_g2r; // Translation from world to range sensor frames

    bool point_volume;
    int points_per_observable;

    std::vector<std::vector<float>> point_pos;

    int max_measurements;
    const int max_measurements_default;
    sensor_msgs::PointCloud2 cloud_out;
    pcl::PointCloud<pcl::PointXYZ> cloud;
    pcl::PointCloud<pcl::PointXYZ> cloud_old;

    ros::NodeHandle nh;

    ros::Publisher pub;
    ros::Subscriber sub;
};

#endif
